...
ATT_RESPONSE_PREFIX = "<|improved response|>\n"
...
# Note that LLama Template does not take in a system message, so we prepend it to the first user message
...
if __name__ == "__main__":
    ...
    for name, template in templates:
        ...
        print(f"chat_simple:")
        x_chat = template.render(messages=chat_simple, eos_token="</s>", bos_token="<s>")
        print(x_chat)
        x_prompt = template.render(messages=chat_user_only, eos_token="</s>", bos_token="<s>", add_generation_prompt=True)
