...
def main():
    ...
    with deepspeed.zero.GatheredParameters(embeddings.weight, modifier_rank=None):
        ...
        # Calculate the average of all existing embeddings and detach it
        existing_embeddings_avg = embeddings.weight[:-1].mean(dim=0).detach()

        # Prepare to update the last row of the embeddings tensor for the new token
        new_embeddings = embeddings.weight.detach().clone()  # Detach and clone the embeddings
